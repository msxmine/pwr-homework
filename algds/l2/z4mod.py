...
if args.stat == None:
    ...
    for n in range(100, 10001, 100):
        for i in range(0,k):
            ...
            tosort = np.array([ str(x) for x in rng.integers(-10000, high=10000, size=n, dtype=np.int64, endpoint=True)])
            params = [2, 4, 8, 16, 32]
            for prm in params:
                mycopy = tosort.copy()
                starttim = process_time()
                cnum, mnum = hybridquicksort(mycopy, lambda x,y: x <= y, prm)
                endtim = process_time()
                plik.write(str(n) + " " + str(cnum) + " " + str(mnum) + " " + str(endtim - starttim) + " hybrid" + str(prm) + "\n")
    plik.close()
