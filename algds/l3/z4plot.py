...
dostepnek = ["rand", "worst"]
cont = {}

for k in dostepnek:
    data = {}

    plik = open("z4out" + k + ".txt", "r")
...
#        if algo != "insertionsort":
            ...
            y4 = [ (cont[k][algo][n][0])/(n*n) for n in x]
            y5 = [ (cont[k][algo][n][0])/(n*math.log2(n)) for n in x]
    ...
    axs[idx,0].set_ylabel("Dataset="+str(k), size="large")
    
axs[0,0].set_title("Porównania")
axs[0,1].set_title("Przestawienia")
axs[0,2].set_title("Czas")
axs[0,3].set_title("Porównania/N^2")
axs[0,4].set_title("Porównania/N*log(N)")
